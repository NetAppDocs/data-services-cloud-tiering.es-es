...
TIP: No es necesario configurar el almacenamiento de objetos porque ya estaba configurado cuando configuró inicialmente la organización en niveles para el clúster.  ONTAP agrupa los datos inactivos de cualquier volumen adicional en el mismo almacén de objetos.
...
Cada agregado en sus sistemas ONTAP locales tiene dos configuraciones que puede ajustar: el umbral de plenitud de niveles y si los informes de datos inactivos están habilitados.
...
Si se producen fallas, Cloud Tiering muestra un estado de salud operativa "Fallido" en el Panel de control del clúster.  La salud refleja el estado del sistema ONTAP y de la NetApp Console.

.Pasos
. Identifique cualquier clúster que tenga un estado operativo de "Fallido".
. Coloque el cursor sobre el ícono informativo "i" para ver el motivo del error.
. Corrija el problema:
+
